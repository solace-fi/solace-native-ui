...
import { DelegateVoteGauge } from './organisms/DelegateVoteGauge'
  ...
  */
  ...
  const canCallVoteMultiple = useMemo(() => {
    const containsChangedOrAddedActiveGauges =
      editingDelegateVotesData.localVoteAllocation.filter((g) => {
  ...
  useEffect(() => {
    if (!isEditing) handleEditingDelegateVotesData(delegateVotesData)
  }, [isEditing])
    ...
    <>
      ...
      <>
        ...
        <Flex col itemsCenter gap={15}>
          <ShadowDiv>
            <Flex gap={12} p={10}>
              <Flex col itemsCenter width={100}>
                ...
                <Text techygradient big3>
                  {truncateValue(formatUnits(editingDelegateVotesData.votePower, 18), 2)}
                </Text>
              </Flex>
              <Flex col itemsCenter width={100}>
                <Text t6s>Used Percentage</Text>
                <Text big3>{(parseFloat(editingDelegateVotesData.usedVotePowerBPS.toString()) / 100).toString()}%</Text>
              </Flex>
          ...
          {editingDelegateVotesData.localVoteAllocation.length > 1 && isEditing && (
            <Flex col gap={5}>
            ...
            isEditing ? (
              <>
                ...
                <Button
                  ...
                  disabled={
                    !canCallVoteMultiple ||
                    editingDelegateVotesData.localVoteAllocation.filter((item) => item.changed).length == 0 ||
                ...
                {editingDelegateVotesData.localVoteAllocation.filter((item) => !item.added).length > 0 && (
                  <Button error widthP={100} onClick={callRemoveVote}>
